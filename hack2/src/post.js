...
function Post(props) {
  ...
  const getPostDetail = async () => {
    const result = await instance.get(`/postDetail?pid=${pid}`);
    if (result.status === 200) setData(result.data.post);
    else setData(null);
  }

  // TODO 5-(2): complete delPost function to delete a post from database
  const delPost = async () => {
    const result = await instance.delete(`/post?pid=${pid}`);
    if (result.status === 200) setData(null);
      ...
      {data ?
        <div className="article-container">
          <div className="article-title" id="pid-detail-title">
            ...
            <IconButton className="post-delete" size="small" id="pid-detail-del-btn" onClick={(e) => delPost()}>
